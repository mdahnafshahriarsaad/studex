  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-between p-6 relative overflow-hidden select-none">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        ...
        <span className="text-xs text-neutral-400 font-bold px-3.5 py-1.5 rounded-full glass-panel border border-white/10">v{APP_INFO.version}</span>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.94, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white mb-4">
          {t('welcome.toStudex', lang)}
        </h1>

        <p className="text-neutral-400 text-base md:text-lg font-normal leading-relaxed max-w-sm mb-8">
          {t('welcome.tagline', lang)}
        </p>

        <div className="w-full space-y-3">
          <Button variant="primary" size="lg" fullWidth icon={<ArrowRight className="w-5 h-5 ml-1" />} onClick={onStartSetup}>
            {t('welcome.startSetup', lang)}
          </Button>
          {onOpenAuthModal && (
            <Button variant="glass" size="lg" fullWidth onClick={onOpenAuthModal}>
              {t('welcome.emailLogin', lang)}
            </Button>
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mt-6 text-xs text-neutral-500">
          ...
          <span>{t('welcome.localSetup', lang)}</span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} transition={{ delay: 0.8 }}
        ...
        <p className="text-[10px] text-neutral-600 mt-1">Developed by {APP_INFO.developers.join(' & ')}</p>
      </motion.div>
    </div>
  );
};
