...
export const GuardianPage: React.FC<GuardianPageProps> = ({ profile, settings, onUpdateProfile }) => {
  ...
  React.useEffect(() => { if (urlCode) { handleConnectWithCode(urlCode); } }, []);

  const handleConnectWithCode = async (codeToConnect: string) => {
    setErrorMsg(null); setSuccessMsg(null);
    if (!codeToConnect.trim()) { setErrorMsg(t('guardian.enterCode', lang)); return; }
    setLoadingConnect(true);
    try { const result = await lookupGuardianStudentAsync(codeToConnect); if (result && result.profile) { setConnectedProfile(result.profile); setConnectedStudentName(result.profile.name); setLoadingConnect(false); return; } } catch (err) { console.warn('Backend guardian lookup failed, trying local:', err); }
    const acc = findAccountByGuardianCode(codeToConnect);
    if (acc) { setConnectedProfile(acc.profile); setConnectedStudentName(acc.name); } else { setErrorMsg(t('guardian.invalidCode', lang)); }
    setLoadingConnect(false);
  };

  const handleFullConnect = async () => {
    if (!inputCode.trim() || !guardianName.trim()) { setErrorMsg(t('guardian.enterNameAndCode', lang)); return; }
    setLoadingConnect(true); setErrorMsg(null);
    try { const result = await connectGuardianAsync(inputCode, guardianName.trim()); if (result.success) { setSuccessMsg(`${t('guardian.connectedSuccess', lang)}`); setConnectedStudentName(result.studentName || null); try { const dashData = await fetchGuardianDashboardAsync(inputCode); if (dashData && dashData.profile) { setConnectedProfile(dashData.profile); } } catch { } } } catch (err: any) { setErrorMsg(err.message || t('guardian.connectFailed', lang)); } finally { setLoadingConnect(false); }
  };

  const handleCopyPasscode = () => { navigator.clipboard.writeText(guardianInfo.guardianCode || guardianInfo.passcode); setCopiedCode(true); setTimeout(() => setCopiedCode(false), 2000); };
  const handleCopyShareLink = () => { navigator.clipboard.writeText(shareableUrl); setCopiedLink(true); setTimeout(() => setCopiedLink(false), 2000); };
  const handleSendEncouragement = () => { if (!encouragementText.trim()) return; setEncouragementSent(true); setEncouragementText(''); setTimeout(() => setEncouragementSent(false), 3000); };

  const handleRefreshDashboard = async () => { if (!inputCode.trim()) return; setLoadingDashboard(true); try { const dashData = await fetchGuardianDashboardAsync(inputCode); if (dashData && dashData.profile) { setConnectedProfile(dashData.profile); } } catch { } finally { setLoadingDashboard(false); } };

  const handleDisconnect = () => { setConnectedProfile(null); setConnectedStudentName(null); setSuccessMsg(null); setErrorMsg(null); setInputCode(''); setGuardianName(''); };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-white">
            {isGuardianView ? `${t('guardian.monitoring', lang)}: ${connectedStudentName || targetProfile.name}` : t('guardian.title', lang)}
          </h1>
          <p className="text-neutral-400 text-sm mt-0.5">
            {isGuardianView ? t('guardian.connectedView', lang) : t('guardian.shareDesc', lang)}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            <div className="space-y-3 border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-6">
              ...
              <div className="p-3 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
                <span className="text-sm font-mono font-extrabold text-electric-400 tracking-wider">{guardianInfo.guardianCode || guardianInfo.passcode || t('dashboard.notGeneratedYet', lang)}</span>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="glass" icon={copiedCode ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />} onClick={handleCopyPasscode}>{copiedCode ? t('guardian.copied', lang) : t('guardian.copyCode', lang)}</Button>
                  <Button size="sm" variant="primary" icon={copiedLink ? <Check className="w-3.5 h-3.5" /> : <Share2 className="w-3.5 h-3.5" />} onClick={handleCopyShareLink}>{copiedLink ? t('guardian.copied', lang) : t('guardian.shareLink', lang)}</Button>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <input type="text" value={guardianName} onChange={(e) => setGuardianName(e.target.value)} placeholder={`e.g. Mr. Rahman`}
                  ...
                  <Button variant="primary" size="md" icon={loadingConnect ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4" />} onClick={handleFullConnect} disabled={loadingConnect}>{t('guardian.connect', lang)}</Button>
                </div>
              </div>
              ...
              {targetProfile.subjects && targetProfile.subjects.length > 0 ? targetProfile.subjects.map((subj) => (
                <div key={subj.id} className="p-3.5 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between text-xs">
                  <div>
                    ...
                    <span className="text-neutral-400">{formatNumber(subj.completedChapters, lang)}/{formatNumber(subj.chapters?.length || 0, lang)} {t('guardian.chapters', lang)} ({formatNumber(subj.completedPages, lang)} / {formatNumber(subj.totalPages, lang)} {t('guardian.pages', lang)})</span>
                  </div>
                ...
                <div className="p-6 text-center text-neutral-500 text-xs">{isGuardianView ? t('dashboard.waitingForData', lang) : t('guardian.noSubjects', lang)}</div>
              )}
            </div>
          </GlassCard>
